using AstroTime: SECONDS_PER_DAY, TDBEpoch, UT1Epoch, fractionofday, j2000, value
...
equinoxes(::IAU2000, ep::Epoch, ϵ, δψ) = δψ * cos(ϵ) + equinoxes(iau2000, ep)

function equinoxes(iau::IAU2000Model, ep::Epoch)
    _, δϵ_pr = precession(iau2000, ep)
    ϵ = obliquity(iau1980, ep) + δϵ_pr
    δψ, _ = nutation(iau, ep)
    return equinoxes(iau2000, ep, ϵ, δψ)
end
...
function mean_sidereal(::IAU1982, ep::Epoch)
    j2k = ep |> UT1Epoch |> j2000
    t = j2k |> centuries |> value
    f = SECONDS_PER_DAY * (value(j2k) % 1.0)
    gmst0 = @evalpoly(
        t,
...
function mean_sidereal(::IAU2000, ep::Epoch)
    t = ep |> TTEpoch |> j2000 |> centuries |> value
    gmst0 = @evalpoly(
        t,
        0.014506,
...
function apparent_sidereal(::IAU1994, ep::Epoch)
    ...
    eqeq94 = equinoxes(iau1994, ep)
    return mod2pi(gmst82 + eqeq94)
end

function apparent_sidereal(iau::IAU2000Model, ep::Epoch)
    ee = equinoxes(iau, ep)
